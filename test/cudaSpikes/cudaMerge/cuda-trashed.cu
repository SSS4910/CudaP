...
//############## TEST AREA ######################################################################
    //trying to make non-pointer array to use in cuda
    /*Request *requests;
    memcpy(&requests, input_buffer->requests, sizeof(Request) * N);

    int i;
    for(i = 0; i < N; i++)
    {
        printf("Host: %s\tReturn code: %d\n", requests[i].host, requests[i].retCode);
    }*/
...
//###########################################################################################
    ...
    // CPU variables
    Buffer *cudaBuffer = (Buffer *)malloc(sizeof(Buffer));
    ...
    //buffer_init(cudaBuffer);
    //cuda_buffer_init(cudaBuffer);

    // creates zero-copy memory for buffer (both CPU and GPU point to same memory). A pointer will be given to the GPU later...
    HANDLE_ERROR( cudaHostAlloc( (void **) &cudaBuffer, sizeof(Buffer), cudaHostAllocWriteCombined | cudaHostAllocMapped));
    
    memcpy(cudaBuffer, input_buffer, sizeof(Buffer));
    
    int j;
    for(j = 0; j < N; j++)
    {
        printf("Host: %s\tReturn Code: %d\n", cudaBuffer->requests[j].host, cudaBuffer->requests[j].retCode);
    }
    ...
    analyze_200<<< blocks, threads, 0, stream0 >>>(dev_buffer, dev_results200, dev_stats200);
